...
select
  	...
  	, round(6371*acos(sind(a.latitude) * sind(a2.latitude) + cosd(a.latitude) * cosd(a2.latitude) * cosd(a.longitude - a2.longitude))::numeric ,0) 	as "L"
  	, ac."range"
  	, case when ac."range" -
                      	round(6371*acos(sind(a.latitude) * sind(a2.latitude) + cosd(a.latitude) * cosd(a2.latitude) * cosd(a.longitude - a2.longitude))::numeric ,0)
                      	> 0 then 'Yes'
  	  	   else 'No'
  	  end as "Enough_range"
from flight_direction fd
